...
interface EditRecipeFormProps {
  recipe: {
    ...
    ispublic: boolean;
    image?: string;
    recipe_ingredients: { ingredient_id: number; amount: number }[];
  };
}
...
export default function EditRecipeForm({ recipe}: EditRecipeFormProps) {
  ...
  const handleAddIngredient = () => {
    setSelectedIngredients([...selectedIngredients, { ingredientId: 0, amount: "" }]);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-(--gray) rounded shadow">
      <h1 className="text-2xl font-bold mb-4">Rezept bearbeiten</h1>
      ...
      <div className="flex gap-4 mt-10">
        ...
        <button
          ...
          className="px-3 py-2 bg-red-600 font-bold text-white rounded cursor-pointer hover:bg-red-700 transition"
        >
          Löschen
        </button>
      </div>
